...
console.log('🚀 Byte UI Development Server with Auto-Restart');
...
function startServer() {
	...
	serverProcess.on('close', (code) => {
		...
		if (code !== 0 && restartCount < maxRestarts) {
			console.log(
				`🔄 Restarting server (attempt ${restartCount + 1}/${maxRestarts})...`,
			);
			restartCount++;
			...
			console.log(
				`❌ Max restart attempts (${maxRestarts}) reached. Please check for errors.`,
			);
			process.exit(1);
		}
	});
